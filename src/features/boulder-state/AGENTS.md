...
Inspected interactively via `bunx oh-my-opencode boulder` (see [`src/cli/boulder/`](src/cli/boulder/)).
...
| Where | What |
|-------|------|
| [`src/cli/boulder/`](src/cli/boulder/) | CLI inspector formats this state |
| [`src/hooks/atlas/`](src/hooks/atlas/) | Reads work state, drives boulder-complete and parallel-delegation prompts |
| [`src/hooks/ralph-loop/`](src/hooks/ralph-loop/) | Resumes subagent task sessions via `task_sessions` |
| [`src/hooks/start-work/`](src/hooks/start-work/) | Creates the BoulderState on `/start-work` invocation |
| [`src/hooks/todo-continuation-enforcer/`](src/hooks/todo-continuation-enforcer/) | Session-idle continuation when boulder incomplete |
