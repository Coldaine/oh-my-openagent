...
function createEventsFile(): string {
  return createTemporaryModelSelectionEventsFile("omo-model-selection-")
}

describe("model-selection-events", () => {
  test("fills timestamp and round-robin strategy without prompt content", () => {
    const input = {
  ...
  test("writes each appended event on its own JSONL line", () => {
  ...
  test("clears the events file for isolated tests", () => {
    const filePath = createEventsFile()
  ...
  test("persists only routing metadata when extra sensitive fields are present", () => {
    const filePath = createEventsFile()
