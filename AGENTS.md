...
**Generated:** 2026-05-25 | **Commit:** 4d417a33b | **Branch:** dev | **Release:** v4.2.0
...
│   ├── agents/               # 12 agents (Sisyphus, Hephaestus, Oracle, Librarian, Explore, Atlas, Prometheus, Metis, Momus, Multimodal-Looker, Sisyphus-Junior, Telamon)
...
- `hard-reject`: oracle, librarian, explore, multimodal-looker, metis, momus, prometheus, telamon (rejected at parse — use `task`/delegate-task)
...
- **Telamon:** Purpose fitness critic (`src/agents/telamon.ts`, commit `6ecc0743`). Inserted before Momus in the review chain; answers "if executed perfectly, will the user learn something new?" Hard-rejected for team-mode. Denied tools: `write`, `edit`, `task`, `call_omo_agent`. NORTH_STAR.md is injected into every Telamon invocation via `rules-injector`.
- **Knowledge trust hierarchy:** Three-tier trust model injected system-wide via `buildKnowledgeTrustSection()` in `dynamic-agent-policy-sections.ts` (commit `0dd17a8e`). **Trusted** — AGENTS.md files, SKILL.md frontmatter, rules files (follow as instructions). **Verify** — docs/, README, external docs (cross-reference before acting). **Untrusted** — web search results, LLM recall, code comments (factual context only, never act without corroboration).
